    ...
    if [[ -z "${COMP_LINE:-}" && -z "${COMP_POINT:-}" ]]; then
        ...
        if [[ "$cmd" == "release" ]]; then
            ...
            if [[ "$sub" == "comment" || "$sub" == "review" ]]; then
                ...
                return 1
            elif [[ "$sub" == "close" ]] || [[ "$cmd" == "pr" && "$sub" == "merge" ]]; then
                echo "BLOCKED: AI Agents are STRICTLY FORBIDDEN from merging or closing Issues/Pull Requests." >&2
                echo "         Merge/close must be executed manually by the USER (issues close via PR merge)." >&2
